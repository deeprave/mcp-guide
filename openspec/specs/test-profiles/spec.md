# test-profiles Specification

## Purpose

Provide testing and verification profiles and documents that compose with language
and platform profiles while keeping build guidance out of the checks category.

## Requirements

### Requirement: Generic testing profile
The system SHALL provide a bundled, selectable `testing` profile that selects the
existing general testing guidance from the `checks` category.
...
The system SHALL provide focused testing guidance under `checks/<language>/<platform>/`
for supported language/platform combinations. Shared framework guidance MAY be grouped
where doing so avoids repetition.
...
The system SHALL provide bundled, selectable `xctest` and `swift-testing` profiles
with focused test-framework guidance. Applying both SHALL preserve both sets of guidance.
