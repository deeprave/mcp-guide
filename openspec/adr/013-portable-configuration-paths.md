...
The same file is copied between hosts, worktrees, and Docker images, which may
anchor it on a user-relative location. This is fully intentional.

Conversion to a path replaces a user-anchored location with a host-specific
filesystem path when it is used and is resolved as a path.
...
- Do not call `.resolve()` or `.absolute()` on a user-supplied path in order
  to "fix" it before rewriting `config.yaml`.
- User-anchored tilde paths that start with `~` and environment variables such
  as `$HOME` are resolved at use time through `LazyPath.resolve()` (expandvars,
  expanduser, then filesystem resolve). That does not authorise rewriting a
  user-anchored path into a host-absolute one.
