...
- Rewrite the combined instruction's wording in a cooperative, second-person, why/what/action style: state that Guide is checking for the OpenSpec CLI's location and version because some OpenSpec-dependent features may be gated by version, collect both values in one agent command invocation, and explain that the result is stored in the global `openspec-state` feature flag (not project-overridable).
