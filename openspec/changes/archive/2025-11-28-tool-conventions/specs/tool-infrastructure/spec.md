...
The system SHALL provide tool collection mechanism in ToolArguments for lazy registration.

#### Scenario: Lazy tool registration
- **WHEN** ToolArguments subclass is instantiated with handler function
- **THEN** tool is registered on first instantiation
- **AND** handler function is associated with args class
- **AND** registration happens at runtime, not import time

#### Scenario: Handler injection via constructor
- **WHEN** args class __init__ is called with handler parameter
- **THEN** handler is stored and class is registered
- **AND** subsequent instantiations do not re-register
- **AND** registration is isolated per test/context

#### Scenario: Collection retrieval
- **WHEN** ToolArguments.get_declared_tools() is called
- **THEN** returns dictionary of all registered tools (args class, handler function)
- **AND** clears collection to prevent double registration
- **AND** operation is thread-safe with asyncio lock

#### Scenario: Test isolation
- **WHEN** tests instantiate tool args
- **THEN** tools are only registered in that test context
- **AND** import does not pollute global state
- **AND** get_declared_tools() clears registry for next test

#### Scenario: Thread safety
- **WHEN** multiple coroutines access collection
- **THEN** asyncio lock protects _declared dictionary
- **AND** no race conditions occur during registration or retrieval
