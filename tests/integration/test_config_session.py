...
from mcp_guide.session import Session, get_session, set_current_session
...
class TestConfigSessionIntegration:
    ...
    async def test_end_to_end_workflow(self, tmp_path):
        ...
        new_session = await Session.create_session("test-project", _config_dir_for_tests=str(tmp_path))
    ...
    async def test_concurrent_sessions_different_projects(self, tmp_path):
        ...
        async def task1():
            session1 = await Session.create_session("project1", _config_dir_for_tests=str(tmp_path))
            ...
            retrieved = await get_session(project_name="project1")
            assert retrieved is session1
            project = await retrieved.get_project()
            results.append(("task1", len(project.categories)))

        async def task2():
            session2 = await Session.create_session("project2", _config_dir_for_tests=str(tmp_path))
            ...
            retrieved = await get_session(project_name="project2")
        ...
        session1_verify = await Session.create_session("project1", _config_dir_for_tests=str(tmp_path))
        session2_verify = await Session.create_session("project2", _config_dir_for_tests=str(tmp_path))
    ...
    async def test_file_locking_prevents_corruption(self, tmp_path):
        ...
        initial_session = await Session.create_session("test-project", _config_dir_for_tests=str(tmp_path))
        ...
        final_session = await Session.create_session("test-project", _config_dir_for_tests=str(tmp_path))
