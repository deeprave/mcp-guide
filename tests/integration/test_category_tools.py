...
from tests.helpers import bind_isolated_test_session, unique_test_project_name
...
async def _get_test_session(runtime, project_name: str = "test") -> Session:
    """Reload a bound Session from the process runtime's configuration."""
    await runtime.start()
...
async def test_category_management_workflow(mcp_server, test_session, monkeypatch):
    ...
    async with Client(FastMCPTransport(mcp_server, raise_exceptions=True), mode="legacy") as client:
        ...
        # List categories
        args = CategoryCollectionListArgs(type="category", verbose=True)
        result = await call_mcp_tool(client, "category_collection_list", args)
        response = json.loads(result.content[0].text)  # type: ignore[union-attr]
        assert response["success"] is True
...
@pytest.mark.parametrize(
    ...
    [
        (
            "invalid_name",
            lambda: CategoryCollectionAddArgs(type="category", name="", dir="src/api", patterns=["*.py"]),
            None,
        ),
        (
            "invalid_patterns",
            lambda: CategoryCollectionAddArgs(type="category", name="api", dir="src/api", patterns=["../*.py"]),
            None,
        ),
        (
            "duplicate",
            lambda: CategoryCollectionAddArgs(type="category", name="api", dir="src/api2", patterns=["*.py"]),
        ...
        (
            "update_nonexistent",
            lambda: CategoryCollectionUpdateArgs(type="category", name="nonexistent", add_patterns=["*.py"]),
            None,
        ),
        ("remove_nonexistent", lambda: CategoryCollectionRemoveArgs(type="category", name="nonexistent"), None),
    ],
...
@pytest.mark.anyio
async def test_category_persists_after_add(mcp_server, test_session, runtime, monkeypatch):
    ...
    session2 = await _get_test_session(runtime, session1.project_name)
...
@pytest.mark.anyio
async def test_category_persists_after_update(mcp_server, test_session, runtime, monkeypatch):
...
@pytest.mark.anyio
async def test_category_removed_persists(mcp_server, test_session, runtime, monkeypatch):
...
@pytest.mark.anyio
async def test_multiple_operations_persist(mcp_server, test_session, runtime, monkeypatch):
...
@pytest.mark.anyio
async def test_category_removal_preserves_collections(mcp_server, test_session, runtime, monkeypatch):
    ...
    async with Client(FastMCPTransport(mcp_server, raise_exceptions=True), mode="legacy") as client:
        ...
        args2 = CategoryCollectionAddArgs(type="category", name="docs", dir="docs", patterns=["*.md"])
        await call_mcp_tool(client, "category_collection_add", args2)
        args3 = CategoryCollectionAddArgs(type="collection", name="backend", categories=["api", "docs"])
        await call_mcp_tool(client, "category_collection_add", args3)
...
@pytest.mark.anyio
async def test_update_category_preserves_collections(mcp_server, test_session, runtime, monkeypatch):
...
async def test_category_content_file_read_error(mcp_server, test_session, tmp_path, monkeypatch):
    ...
    # Create a matching file so discovery succeeds before the read step fails.
    docroot = Path(await get_runtime().get_docroot()) / "docs"
...
async def test_category_list_files_success(mcp_server, test_session, tmp_path, monkeypatch):
    ...
    await session.update_config(lambda p: p.with_category("guide", Category(dir="guide", patterns=["guidelines*"])))
...
async def test_category_list_files_mixed_file_types(mcp_server, test_session, tmp_path, monkeypatch):
    ...
    (guide_dir / "subdir").mkdir(parents=True, exist_ok=True)
