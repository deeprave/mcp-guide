...
def test_parse_stdio_mode():
    ...
    mode, host, port = parse_transport_mode("stdio")
    assert mode == "stdio"
    assert host is None
    assert port is None
...
def test_parse_http_shorthand():
    ...
    mode, host, port = parse_transport_mode("http")
    assert mode == "http"
...
def test_parse_https_shorthand():
    ...
    mode, host, port = parse_transport_mode("https")
    assert mode == "https"
    assert host == "0.0.0.0"
    assert port == 443
...
def test_parse_http_with_host_and_port():
    ...
    mode, host, port = parse_transport_mode("http://localhost:8080")
    assert mode == "http"
    assert host == "localhost"
    assert port == 8080
...
def test_parse_https_with_host_and_port():
    ...
    mode, host, port = parse_transport_mode("https://example.com:8443")
    assert mode == "https"
...
def test_parse_http_with_port_only():
    ...
    mode, host, port = parse_transport_mode("http://:8080")
    assert mode == "http"
    assert host == "localhost"
    assert port == 8080
...
def test_parse_https_with_port_only():
    ...
    mode, host, port = parse_transport_mode("https://:8443")
    assert mode == "https"
    assert host == "0.0.0.0"
    assert port == 8443
...
def test_parse_http_with_host_only():
    ...
    mode, host, port = parse_transport_mode("http://example.com")
    assert mode == "http"
    assert host == "example.com"
    assert port == 8080
...
def test_parse_https_with_host_only():
    ...
    mode, host, port = parse_transport_mode("https://example.com")
    assert mode == "https"
    assert host == "example.com"
    assert port == 443
