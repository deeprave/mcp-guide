...
@pytest.mark.anyio
async def test_native_skill_resource_preserves_arbitrary_query_arguments(tmp_path, monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_negotiated_skills_list_returns_only_bound_session_skills(tmp_path, monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_skills_list_change_notification_requires_an_effective_change(tmp_path, monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_skills_list_change_waits_for_the_owning_sessions_request_context(tmp_path, monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_skills_list_subscription_survives_project_switch(tmp_path, monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_skills_list_rejects_a_client_that_did_not_negotiate(tmp_path, monkeypatch) -> None:
