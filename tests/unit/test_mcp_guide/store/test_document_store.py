...
async def test_upsert_updates_content_and_timestamp(db):
    first = (await add_document("docs", "readme", "/path/readme.md", "file", "v1", mtime=100.0, db_path=db)).record
    second = (await add_document("docs", "readme", "/path/readme.md", "file", "v2", mtime=200.0, db_path=db)).record
...
@pytest.mark.anyio
async def test_store_round_trip_metadata_listing_and_removal(db):
    assert await get_document("docs", "readme", db_path=db) is None
    assert await get_document_content("docs", "readme", db_path=db) is None
    assert not await remove_document("docs", "readme", db_path=db)
    meta = {"etag": "abc123", "content-type": "text/markdown"}
    result = await add_document(
        "docs", "readme", "/path/readme.md", "file", "# Hello", metadata=meta, mtime=1700000000.5, db_path=db
    )
    assert not result.skipped
    record = result.record
    assert (record.category, record.name, record.source_type, record.content) == ("docs", "readme", "file", "# Hello")
    ...
    assert fetched.mtime == record.mtime
    assert await get_document_content("docs", "readme", db_path=db) == "# Hello"
    other = await add_document("docs", "another", "/another", "file", "Another", db_path=db)
    assert other.record.mtime is None
    await add_document("other", "external", "https://example.test", "url", "External", db_path=db)
    category = await list_documents("docs", db_path=db)
    assert [(item.name, item.mtime, item.content) for item in category] == [
        ("another", None, None),
        ("readme", 1700000000.5, None),
    ]
    all_records = await list_documents(db_path=db)
    assert [(item.category, item.name) for item in all_records] == [
        ("docs", "another"),
        ("docs", "readme"),
        ("other", "external"),
    ]
    assert all(item.content is None for item in all_records)
    assert await remove_document("docs", "readme", db_path=db)
    assert await get_document("docs", "readme", db_path=db) is None
    assert await get_document_content("docs", "readme", db_path=db) is None
    assert not await remove_document("docs", "readme", db_path=db)
    assert [item.name for item in await list_documents("docs", db_path=db)] == ["another"]
