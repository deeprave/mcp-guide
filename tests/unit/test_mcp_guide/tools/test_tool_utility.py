...
from mcp_guide.tools.tool_utility import GetClientInfoArgs, get_client_info


@pytest.mark.asyncio
async def test_get_client_info_no_cache():
    """Test get_client_info detects and caches agent info."""
    # Create mock context
    ctx = Mock()
...
@pytest.mark.asyncio
async def test_get_client_info_with_cache():
    """Test get_client_info returns cached agent info."""
...
@pytest.mark.asyncio
async def test_get_client_info_no_client_params():
    """Test get_client_info handles missing client_params."""
    ctx = Mock()
    ctx.fastmcp = GuideMCP(name="test-server")
    ctx.fastmcp.agent_info = None
    ctx.session = Mock()
