...
class TestValidateCategoriesExist:
    ...
    def test_some_categories_missing_raises_error(self, test_project):
        ...
        assert all(err["field"] == "categories" for err in exc_info.value.errors)
        assert "missing1" in exc_info.value.errors[0]["message"]
        assert "missing2" in exc_info.value.errors[1]["message"]
